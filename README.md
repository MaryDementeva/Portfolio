...
Задача: разработать алгоритм переноса художественного стиля между изображениями.
Решение: Настроен алгоритм на основе VGG-19 с оптимизацией content loss и style loss. Подобраны весовые коэффициенты для баланса содержания и стиля (PyTorch).
Результат: Успешный перенос различных художественных стилей на пользовательские фото с сохранением узнаваемости исходного содержания.
...
Решение: Реализован итеративный процесс обучения параметров шума с применением метода градиентного подъема (аниградиент) (PrTorch).

Результат: Модель успешно вводится в заблуждение: исходное изображение с добавленным шумом стабильно классифицируется как ложный класс с высокой уверенностью (>95%). Визуально изображение остаётся узнаваемым для человека.
